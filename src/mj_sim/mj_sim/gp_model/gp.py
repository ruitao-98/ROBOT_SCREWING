...
class CustomKernelFunctions:
    ...
    def __init__(self, kernel_func, params=None):
        ...
        if self.kernel_type == 'squared_exponential':
            ...
            self.kernel = self.squared_exponential_kernel  # 将方法本身（作为一个对象）赋值给实例属性 self.kernel，self.kernel 变成了一个绑定方法，绑定到了当前实例 self 上
            #self.kernel 提供了一个抽象层，允许在 __init__ 中根据 kernel_type 动态绑定不同的核函数。
        else:
            raise NotImplementedError("only squared_exponential is supported")

        self.theta = self.get_trainable_parameters()

    def __call__(self, x_1, x_2): #__call__ 是类的特殊方法，使实例可调用（例如 obj(a, b)）
        return self.kernel(x_1, x_2)

    def __str__(self): #为平方指数核（Squared Exponential Kernel，即 RBF 核）生成了一个格式化的字符串，展示了核函数的超参数（信号方差 sigma_f 和长度尺度 l）。
        """
    ...
    @staticmethod  #内置修饰器 用于将类中的方法定义为静态方法
    ...
    def diff(self, z, z_train):
        ...
        return - k_zZ * (z_tile - z_train).T / cs.mtimes(cs.MX.ones(z_train.shape[0], 1), len_scale).T #符号矩阵（casadi.MX 类型）
...
class CustomGPRegression:
    ...
    @property  #用于将类中的方法变成一个只读属性。它允许你以访问属性的方式（不加括号）调用方法，从而隐藏方法的实现细节，提供更直观的接口。obj.property
    def kernel(self):
        """
        将方法伪装成属性，访问时不需要加 ()。
