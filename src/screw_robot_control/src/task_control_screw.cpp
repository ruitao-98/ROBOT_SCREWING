...
ScrewControlServer::ScrewControlServer() : Node("task_control") {
  RCLCPP_INFO(this->get_logger(), "Task Control Server started.");
...
void ScrewControlServer::execute(
    ...
    if (num == 0) {
      ...
      std::this_thread::sleep_for(std::chrono::milliseconds(300));// 0.1s
      feedback->screw_status = 1.0;
      goal_handle->publish_feedback(feedback);
