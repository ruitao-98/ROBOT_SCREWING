...
class Dual_arm_env(gym.Env):
    def __init__(self, render=True):
        ...
        self.adm_k = np.array([1200, 1200, 1200, 1.0, 1.0, 1.0])  # [10 10 10 10 10 10]
        self.adm_m = np.array([3.0, 3.0, 3.0, 1.0, 1.0, 1.0])
        ...
        self.select_vector = np.array([0, 0, 0, 1, 1, 1])
        ...
        # 转换为 Python 列表
        self.start_pos_list = self.start_pos.tolist()  # 转换为 [x, y, z] 列表
        self.start_rotm_flat = self.start_rotm.flatten().tolist()  # 展平为 [r11, r12, r13, r21, r22, r23, r31, r32, r33] 列表
        self.rotation_speed = 46.0
        ...
        self.srv = self.node.create_service(StartRotation, 'get_rotation_param', self.ready_callback)
    ...
    def ready_callback(self, request, response):
        ...
        response.rotation_speed = self.rotation_speed # 旋拧工具的旋拧速度
        response.screw_pitch = self.screw_pitch
        response.pos_para = self.start_pos_list   # 轨迹初始位置
        response.ori_para = self.start_rotm_flat  # 轨迹初始姿态
        return response
    ...
    def update_rotation(self, angular_disp, current_rotm):
        """
        更新旋转矩阵基于给定的角位移。
        :param angular_disp: 角位移向量 (numpy array, 3D)
        :param current_rotm: 当前的 3x3 旋转矩阵 (numpy array)
        :return: 新的旋转矩阵 (numpy array, 3x3)
        """
        # 计算角位移的模（旋转角度）
        angle = np.linalg.norm(angular_disp)
        
        # 归一化角位移向量（旋转轴）
        if angle != 0:
            axis = angular_disp / angle
        else:
            axis = np.array([1.0, 0.0, 0.0])  # 若角度为 0，设置默认轴

        # 创建旋转对象（AngleAxis 等价）
        delta_rotation = R.from_rotvec(axis * angle)
        
        # 将增量旋转转换为旋转矩阵并与当前旋转矩阵相乘
        new_orientation = delta_rotation.as_matrix() @ current_rotm 
    ...
    def admittance_control(self, ctl_ori=False):
        ...
        angle = rotation.magnitude()  # Rotation angle
        axis = rotation.as_rotvec() / angle if angle != 0 else np.zeros(3)  # Rotation axis
        # Compute rotation vector (axis * angle)
        rotation_vector = angle * axis
        # Assign to the last 3 elements of e (assuming e is a numpy array)
        e[3:] = rotation_vector.copy()

        e_dot = eef_vel - eef_desired_vel
        MA = force_error - np.multiply(self.adm_k, e) - np.multiply(self.adm_d, e_dot)
        ...
        linear_disp = np.multiply(self.select_vector[:3], linear_disp)
        angular_disp = np.multiply(self.select_vector[3:], angular_disp)
    ...
    def admittance_step_test(self, action_pose, action_vel, stop):
        ...
        # 不固定rotm
        print("linear_disp", linear_disp)
        print("angular_disp", angular_disp)
        print("eef_rotm", eef_rotm)
        new_angular = self.update_rotation(angular_disp, eef_rotm)
    ...
    def reset(self):
        ...
        # desired_pos = np.array([0, 0.47, 0.075])  #路径跟随场景
        desired_pos = np.array([0, 0.5, 0.2])
...
def main():
    ...
    num_points = 1
    control_dt = 0.008
    speed =  ((env.rotation_speed / 6.238) / 60) * env.screw_pitch
    position = env.start_pos
    orientation = env.start_rotm
    ...
    for _ in range(100000):
        ...
        trajectory_positions, trajectory_orientations, trajectory_velocities, trajectory_angular_velocities = \
            traj.online_generate_straight_trajectory(num_points, control_dt, speed, position, orientation, env.eef_pos)

        action_pose[:3] = trajectory_positions[0]
        action_vel[:3] = trajectory_velocities[0]
        stop=1
        print("current_idx = ", t)
